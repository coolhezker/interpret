...
using D = Dir;
    ...
    void D::addChild(const string& n) { // добавить директорию-ребЄнка(вектор childs)
        if (this) {
    ...
    const string& D::returnFileContent(const string& n) const { // вывод строки content типа File с name == n
        if (this) {
            ...
            for (int i = 0; i < this->files.size(); i++) {
                if (this->files[i]->getName() == n) {
                    return this->files[i]->returnFileContent(); // эта функци€ вызываетс€ из класса File(никакой рекурсии нету)
                    isFind = true;
                    break;
                }
            }
    ...
    Dir* D::changeDir(const string& s) { // возврат указател€ на директорию с именем s
        if (this) {
